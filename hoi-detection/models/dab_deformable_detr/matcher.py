...
class HungarianMatcherHOI(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if not self.no_obj:
            ...
            gamma = 2.0
            neg_cost_class = (
                (1 - alpha) * (out_obj_prob**gamma) * (-(1 - out_obj_prob + 1e-8).log())
            )
            pos_cost_class = (
                alpha * ((1 - out_obj_prob) ** gamma) * (-(out_obj_prob + 1e-8).log())
            )
            cost_obj_class = (
                pos_cost_class[:, tgt_obj_labels] - neg_cost_class[:, tgt_obj_labels]
            )
        else:
        ...
        cost_obj_bbox = torch.cdist(out_obj_bbox, tgt_obj_boxes, p=1) * (
            tgt_obj_boxes != 0
        ).any(dim=1).unsqueeze(0)
        if cost_sub_bbox.shape[1] == 0:
            cost_bbox = cost_sub_bbox
        else:
            cost_bbox = torch.stack((cost_sub_bbox, cost_obj_bbox)).max(dim=0)[0]

        cost_sub_giou = -generalized_box_iou(
            box_cxcywh_to_xyxy(out_sub_bbox), box_cxcywh_to_xyxy(tgt_sub_boxes)
        )
        cost_obj_giou = -generalized_box_iou(
            box_cxcywh_to_xyxy(out_obj_bbox), box_cxcywh_to_xyxy(tgt_obj_boxes)
